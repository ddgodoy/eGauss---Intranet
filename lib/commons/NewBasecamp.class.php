...
class NewBasecamp
{
	 ...
	 * @return array
	 */
	public static function getResumenByProyecto($id_proyecto)
	{
		$arDatos  = array();
		$basecamp = self::getBasecampInstance();
		//
	  ...
	  if (count($todo_list) > 0)
	  {
	  	foreach ($todo_list as $lista)
	  	{
		  	...
		  	foreach ($todos->todos->remaining as $todo)
		  	{
		  		...
		  		$aTodo[$todo->id]['asigne'] = $todo->assignee->name;
