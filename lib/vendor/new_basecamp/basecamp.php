...
function basecamp_api_client($appName, $contactInfo, $accountId, $username, $password, $logger = NULL)
{
  if (is_null($logger))
  {
  ...
  return
  function($method, $path, $params=array(), &$response_headers=array()) use ($baseUrl, $credentials, $helloHeader, $logger)
  {
    ...
    $logger->debug("About to send API request:\n" .print_r(compact('method', 'url', 'query', 'payload', 'request_headers'), 1));
