...
<div class="content">
  ...
  <div class="mapa">
		<a href="<?php echo url_for('home/index') ?>"><strong><?php echo __('Home') ?></strong></a>&nbsp;&gt;&nbsp;<?php echo __('Productos') ?>
	</div>
  <h1 class="titulos">
  	<?php echo __('Lista de Productos') ?>
        <?php if($sf_user->hasCredential('super_admin')): ?> 
  	<input type="button" value="<?php echo __('Registrar Productos') ?>" style="float:right;" class="boton" onclick="document.location='<?php echo url_for($str_module.'-register') ?>';"/>
        <?php endif; ?>
  </h1>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="listados">
      <tr>
      	<?php if (count($oList) > 0): ?>
                <th width="10%"><a href="<?php echo $head_link.'&o=p.created_at&s='.$sort ?>"><?php echo __('Date') ?></a></th>
	        <th width="63%"><a href="<?php echo $head_link.'&o=p.name&s='.$sort ?>"><?php echo __('Name') ?></a></th>
	        <th width="4%"></th>
	        <?php if($sf_user->hasCredential('super_admin')): ?>
                <th width="4%"></th>
	        <th width="4%"></th>
      ...
      <tr class="<?php if (!empty($odd)) { echo 'gris'; $odd=0; } else { echo 'blanco'; $odd=1; } ?>"> 
        <td><?php echo Common::getFormattedDate($item->getCreatedAt(),'d/m/Y') ?></td>
