  ...
  <h1 class="titulos">
  	<?php echo __('Lista de Eventos') ?>
        <?php if ($sf_user->hasCredential('super_admin')): ?>
        	<?php
        		$rg_year   = !empty($sch_year)  ? $sch_year  : date('Y');
        		$rg_month  = !empty($sch_month) ? $sch_month : date('m');
        		$rg_day    = !empty($sch_day)   ? $sch_day   : date('d');
        		$rg_params = 'sch_year='.$rg_year.'&sch_month='.$rg_month.'&sch_day='.$rg_day;
        	?>
          <input type="button" value="<?php echo __('Registrar Evento') ?>" style="float:right;" class="boton" onclick="document.location='<?php echo url_for('@'.$str_module.'-register?'.$rg_params) ?>';"/>
        <?php endif; ?>    
  </h1>
      ...
      <tr class="<?php if (!empty($odd)) { echo 'gris'; $odd=0; } else { echo 'blanco'; $odd=1; } ?>">
        ...
        <td align="center">
          <?php $show_module = $item->getTypeCalendarId()==2?'shareholders': $str_module ?>
        	<a href="<?php echo url_for('@'.$show_module.'-show?id='.$item->getId().'&sch_year='.$item->getYear().'&sch_month='.$item->getMonth().'&sch_day='.$item->getDay()) ?>">
        ...
        <?php if($sf_user->hasCredential('super_admin')): ?>
        <td align="center">
        	<a href="<?php echo url_for('@'.$str_module.'-edit?id='.$item->getId().'&sch_year='.$item->getYear().'&sch_month='.$item->getMonth().'&sch_day='.$item->getDay()) ?>">
        		<img border="0" src="/images/editar.png" alt="<?php echo __('Edit') ?>" title="<?php echo __('Edit') ?>">
        	</a>
        </td>
        <td align="center">
        	<a href="<?php echo url_for('@'.$str_module.'-delete?id='.$item->getId().'&sch_year='.$item->getYear().'&sch_month='.$item->getMonth().'&sch_day='.$item->getDay()) ?>" onclick="return confirm('<?php echo __('Are you sure?') ?>');">
